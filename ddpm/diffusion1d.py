...
class Diffusion1d(object):
    ...
    def _get_index_from_list(
        self, vals: torch.Tensor, t: torch.Tensor, x_shape: torch.Size
    ) -> torch.Tensor:
    ...
    def forward_obs(self, x_0, y_o, t):
        ...
        # mean + variance
        return sqrt_alphas_cumprod_t.to(self.device) * y_o.to(
            self.device
        ) + sqrt_one_minus_alphas_cumprod_t.to(self.device) * noise.to(
            self.device
        ), noise.to(
    ...
    @torch.no_grad()
    def sampling_z(self, x_shape: torch.Size) -> np.ndarray:
        x_T = torch.randn(x_shape).to(self.device)
        sampled_tensor = self.sampling(x_T)
        sampled_seq = sampled_tensor.squeeze().detach().cpu().numpy()
        return sampled_tensor, sampled_seq

    @torch.no_grad()
    def sampling_up(self, human_tensor, upper_tensor, t) -> np.ndarray:
        x_t = torch.cat([human_tensor, upper_tensor], dim=2)
        sampled_tensor = self.sample_timestep(x_t, t)
        sampled_seq = sampled_tensor.squeeze().detach().cpu().numpy()
        return sampled_tensor, sampled_seq
    ...
    def gradient_F_observation(self, x, y_o, t):
        ...
        return (
            -2 * torch.matmul((torch.matmul(x, self.H.T) - y_t), self.H) / x.shape[-1]
        )
    ...
    def sample_timestep_guidance(self, x_f, y_o, s_f, s_o, x, t):
        ...
        posterior_variance = self._get_index_from_list(
            self.posterior_variance, t, x.shape
        )
        sqrt_recip_alphas_cur = self._get_index_from_list(
            self.sqrt_recip_alphas, t, x.shape
        )
        alphas_cumprod_cur = self._get_index_from_list(self.alphas_cumprod, t, x.shape)
        alphas_cumprod_prev = self._get_index_from_list(
        ...
        sigma_t = (
            1.0
            * ((1 - alphas_cumprod_prev) / (1 - alphas_cumprod_cur)) ** 0.5
            * ((1 - alphas_cumprod_cur / alphas_cumprod_prev)) ** 0.5
        )
        # sigma_t = 0
        predicted_x0 = (
            alphas_cumprod_prev**0.5
            * (x - sqrt_one_minus_alphas_cumprod_t * eps)
            / (alphas_cumprod_cur**0.5)
        )
        ...
        return (
            predicted_x0 + dir2_xt + posterior_variance * (dFo + dFf) + sigma_t * noise
        )
    ...
    def sampling_guided(self, x_T: torch.Tensor, x_f, y_o, s_f, s_o) -> torch.Tensor:
        ...
        eta = 1
        ts = (
            torch.linspace(self.time_steps, 0, (self.sample_steps + 1))
            .to(self.device)
            .to(torch.long)
        )
        # for i in tqdm(reversed(range(self.time_steps)), desc="Sampling"):
