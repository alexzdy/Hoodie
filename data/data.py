...
def split(path="./human-upper-2048", train_ratio=0.9, test_ratio=0.1):
    list_all = os.listdir(path)

    random.shuffle(list_all)
    list_all_sort = sorted(list_all, key=lambda x: (int(x.split('-')[0]), int(x.split('-')[1].split('.')[0])))
    # i = 1
    # train_list = []
    # test_list = []
    ...
    train_list = sorted(train_list, key=lambda x: (int(x.split('-')[0]), int(x.split('-')[1].split('.')[0])))
    test_list = list_all[int(train_ratio*length):]
    test_list = sorted(test_list, key=lambda x: (int(x.split('-')[0]), int(x.split('-')[1].split('.')[0])))
