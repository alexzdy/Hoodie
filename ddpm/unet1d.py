...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, context=None):
        ...
        q, k, v = map(
            lambda t: rearrange(t, "b (h c) n -> b h c n", h=self.heads), (q, k, v)
        )
...
class Unet1d(nn.Module):
    def __init__(
                    ...
                    [
                        ...
                        Residual(
                            PreNorm(
                                dim_in,
                                CrossAttention(
                                    dim_in, dim_head=attn_dim_head, heads=attn_heads
                                ),
                            )
                        ),
                        block_klass(dim_in, dim_in, time_emb_dim=time_dim),
        ...
        self.mid_attn = Residual(
            PreNorm(
                mid_dim,
                CrossAttention(mid_dim, dim_head=attn_dim_head, heads=attn_heads),
            )
        )
                    ...
                    [
                        ...
                        Residual(
                            PreNorm(
                                dim_out,
                                CrossAttention(
                                    dim_out, dim_head=attn_dim_head, heads=attn_heads
                                ),
                            )
                        ),
                        block_klass(dim_out + dim_in, dim_out, time_emb_dim=time_dim),
