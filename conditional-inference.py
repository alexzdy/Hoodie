...
parser.add_argument("--log_root", type=str, default="./log/Conditional-inference")
...
parser.add_argument(
    ...
    help="[human-upper-2048, human-dress-2048]",
)
parser.add_argument(
    "--stage1_human",
    type=str,
...
if args.logging:
    ...
    logger = get_logger("Conditional-inference", log_dir)
    writer = torch.utils.tensorboard.SummaryWriter(log_dir)
    ckpt_mgr = CheckpointManager(log_dir)
    log_hyperparams(writer, args)
else:
    logger = get_logger("Conditional-inference", None)
    writer = BlackHole()
    ckpt_mgr = BlackHole()
logger.info(args)
# Statistic
stats_save_path_human = os.path.join(
    "./data",
    "human-%s-2048_stats" % args.categories[0].split("-")[1],
    "stats_human-%s-2048.pt" % args.categories[0].split("-")[1],
)
stats_save_path_garment = os.path.join(
    "./data",
    "garment-%s-2048_stats" % args.categories[0].split("-")[1],
    "stats_garment-%s-2048.pt" % args.categories[0].split("-")[1],
)
...
# Sample
dataset_path = os.path.join("./data", args.categories[0])
file_list = os.listdir(dataset_path)
human_pc = []
for file_name in file_list:
    if file_name.endswith(".xyz"):
        file_path = os.path.join(dataset_path, file_name)
        xyz_data = np.loadtxt(file_path)
        human_pc.append(xyz_data)
human_pc = (
    torch.from_numpy(np.stack(human_pc, axis=0))[: args.num_samples, :, :]
    .to(torch.float32)
    .to(args.device)
)
z1 = model_human.pc2z(human_pc, flexibility=args.flexibility)[0]
z1 = z1.unsqueeze(1)
z2_noise = torch.randn(z1.shape).to(args.device)
for j in tqdm(reversed(range(1000)), desc="Sampling"):
    t_list = torch.full((z1.shape[0],), j, dtype=torch.int64, device=args.device)
    z1_noise = diffuser.forward(z1, t_list)[0]
    z0_tensor = sampler(z1_noise, z2_noise, t_list)[0]
    _, z2_noise = z0_tensor.split([args.latent_dim, args.latent_dim], dim=-1)
z2 = z2_noise
