...
parser.add_argument("--log_root", type=str, default="./log/Conditional-generation")
...
if args.logging:
    ...
    logger = get_logger("Conditional-generation", log_dir)
    writer = torch.utils.tensorboard.SummaryWriter(log_dir)
    ckpt_mgr = CheckpointManager(log_dir)
    log_hyperparams(writer, args)
else:
    logger = get_logger("Conditional-generation", None)
    writer = BlackHole()
    ckpt_mgr = BlackHole()
logger.info(args)
# Statistic
...
# Sample
w = torch.randn([args.num_samples, args.latent_dim]).to(args.device)
z1 = model_human.sample_z(w).unsqueeze(1)
